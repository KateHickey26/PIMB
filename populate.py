...
def populate():
    p = open('static/images/default.jpg', 'r')
    ...
    speakers = [{'name':'Adam Scott','desc': 'Are you looking for someone to bring some flair to your next corporate event? Look no further!','rate': 3, 'Tags':['Corporate Events'], 'email':'lucy@munroesmotivators.com','phone':'0563 456 66','company':'Munroes Motivators','hourlyrate':'£500','twitter':'https://twitter.com/home','picture':p},
    {'name':'Ben Halpert','desc': 'I specialise in convincing unemployed people to stop looking for a job, and start giving me money (taken bi-monthly) to help them acheive their dreams!!','rate': 4,'Tags':['','Job Fairs'], 'email':'hnelson@nelsontalent.com','phone':'0173 466 56','company':'Nelson Talent','hourlyrate':'£250','twitter':'https://twitter.com/home','picture':p},
    ...
    ,'rate': 5, 'Tags':['Wellness Retreats','Hotel Conference Room Fillers','Lonely Hearts Conventions'], 'email':'grahem@ghmanagement.com','phone':'0123 456 56','company':'GH Management','hourlyrate':'£300','twitter':'https://twitter.com/home','picture':p},
    {'name':'Donald Sax','desc': 'Self Care is very important to me. This is why my life goals is to charge other a fair, nominal fee for a weekend retreat where you can learn to meditate, to channel good vibes, and to sit cross-legged on the ground.','rate': 1,'Tags':['Wellness Retreats'], 'email':'alice@allstarspeakers.com','phone':'0123 567 56','company':'All Star Speakers','hourlyrate':'£90','twitter':'https://twitter.com/home','picture':p}]
    for s in speakers:
        add_speakers(s['name'], s['desc'],s['Tags'],s['email'],s['phone'],s['company'],s['hourlyrate'],s['twitter'],s['picture'])
...
def add_speakers(name, desc, tags , email, phone, company, hourlyrate, twitter,picture):
    try:
